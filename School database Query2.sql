...
-- Basic queries
SELECT * FROM STUDENTS ORDER BY FIRST_NAME DESC;   -- Fetch all columns and all records (rows) from table.
...
UPDATE STAFF SET FIRST_NAME = 'Rashidah' WHERE STAFF_ID = 'STF1001';
...
SELECT STAFF_TYPE FROM STAFF ; -- Fetches only the first 5 records from the result.
...
-- TO_CHAR / TO_DATE:
SELECT * FROM STUDENTS WHERE DOB = '04-20-2014';
SELECT * FROM STUDENTS WHERE DOB = '13-JAN-2014';
...
SELECT DISTINCT STF.FIRST_NAME, LAST_NAME--, CLS.CLASS_NAME
FROM SUBJECTS SUB
...
SELECT COUNT(DISTINCT ADDRESS_ID) AS TOTAL FROM ADDRESS ; 
...
SELECT SC.CLASS_ID, COUNT(1) AS "Number of students"
...
--Sub-queries
SELECT SS.* FROM STAFF_SALARY SS
JOIN (SELECT AVG(SALARY) SAL FROM STAFF_SALARY) AVG_SAL
ON SS.SALARY > AVG_SAL.SAL;


--Oldest staff members in either gender
-- Multiole column multiple row subquery
...
SUB ON S.GENDER = SUB.GENDER AND S.AGE = SUB.MAX_AGE;




SELECT AVG(AGE) AS AVG_AGE FROM STAFF;
